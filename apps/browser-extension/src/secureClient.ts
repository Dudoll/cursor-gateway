...
export class SecureGatewayClient {
  ...
  async addMemory(input: {
    ...
    if (base.scope === "workspace" && !base.workspaceId) {
      throw new Error("workspace_required");
    }
    const plaintext = e2eeMemoryPayloadSchema.parse({
      protocol: E2EE_PROTOCOL,
      kind: "memory",
      memoryId,
      scope: base.scope,
    ...
    const response = await this.api.post<{ memory: unknown }>(
      "/api/e2ee/v1/memory",
      { envelope }
    );
    return this.decryptMemory(e2eeMemoryRecordSchema.parse(response.memory));
  }

  async archiveAndScrubLegacyData() {
    const [conversationResponse, memoryResponse] = await Promise.all([
      this.api.get<{ conversations: unknown[] }>("/api/conversations?limit=1000"),
      this.api.get<{ facts: unknown[] }>("/api/memory?limit=10000")
    ]);
    if (
      conversationResponse.conversations.length >= 1_000 ||
      memoryResponse.facts.length >= 10_000
    ) {
      throw new Error("legacy_migration_limit_reached");
    }
    const conversations = conversationResponse.conversations.map((value) =>
      conversationSchema.parse(value)
    );
    const memory = memoryResponse.facts.map((value) => memoryFactSchema.parse(value));
    ...
    for (const conversation of conversations) {
      const response = await this.api.get<{ runs: unknown[] }>(
        `/api/conversations/${conversation.id}/runs`
      );
      archivedConversations.push({
        conversation,
        runs: response.runs.map((value) => runRecordSchema.parse(value))
      });
    }

    if (archivedConversations.length === 0 && memory.length === 0) {
      throw new Error("no_legacy_plaintext_data");
    }
    const archive = await this.keys.archiveLegacyData({
      version: 1,
    ...
    const scrubbed = await this.api.post<{
      conversations: number;
      runs: number;
      memory: number;
    }>("/api/e2ee/v1/migrations/scrub-legacy", {
      archiveId: archive.id,
      conversationIds: conversations.map((conversation) => conversation.id),
      memoryIds: memory.map((fact) => fact.id),
      acknowledgement: "local-encrypted-archive-verified"
    });
    return { archive, scrubbed };
  }
