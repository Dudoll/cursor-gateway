...
- 版本号：`apps/desktop/src-tauri/tauri.conf.json`（当前 `0.1.8`），与 `package.json` / `Cargo.toml` /
...
- 打 tag 触发并附加到 Release：`git tag desktop-v0.1.8 && git push origin desktop-v0.1.8`。
...
- 单一事实源：`apps/desktop/src-tauri/tauri.conf.json` 的 `version`（当前 `0.1.8`），
  与 `apps/desktop/package.json`、`Cargo.toml` 保持一致。
- 发布用 tag `desktop-v<semver>`（如 `desktop-v0.1.8`）触发 CI 出包并附加到 Release，
