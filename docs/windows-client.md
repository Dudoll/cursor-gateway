...
- 版本号：`apps/desktop/src-tauri/tauri.conf.json`（当前 `0.1.9`），与 `package.json` / `Cargo.toml` /
...
- 打 tag 触发并附加到 Release：`git tag desktop-v0.1.9 && git push origin desktop-v0.1.9`。
...
- 单一事实源：`apps/desktop/src-tauri/tauri.conf.json` 的 `version`（当前 `0.1.9`），
  与 `apps/desktop/package.json`、`Cargo.toml` 保持一致。
- 发布用 tag `desktop-v<semver>`（如 `desktop-v0.1.9`）触发 CI 出包并附加到 Release，
...
客户端会在启动、登录完成、窗口恢复、网络恢复及定时周期检查公开、非敏感的权威清单
`raw.githubusercontent.com/.../desktop-version.json`，并在登录后回退检查
`GET /api/desktop/version`。清单固定 schema、安装地址和 SHA256；HTML fallback、
错误 Content-Type、缺 hash 或不受信任 URL 均会被拒绝。远端版本更高且安装包可用时，
右上角显示唯一的**向上箭头**。
点击后会先完成登录（如需要），再下载 `/api/desktop/download`、核对发布 SHA256 并启动 NSIS。

Passkey 不在 `tauri.localhost` 执行；客户端会打开
`https://secure.joelzt.org` 的 HTTPS 顶层窗口，由签名桌面壳注入最小 WebAuthn
适配器（不依赖远端静态页是否已更新），在与 RP ID 匹配的 origin 完成 Windows Hello，
再通过受限 Tauri IPC 返回公开验证结果。

### 网络韧性与诊断日志

- Access 登录要求连续两次成功；等待登录时每约 2 秒检查，网络瞬断按 0.5–8 秒指数退避，
  最多连续 8 次、总计最多 5 分钟。HTTP 403 和配置错误立即停止；用户可取消。
- Windows WebView2 禁用 QUIC，保留 HTTPS/H2，避免已确认的 h3 alternative-service 瞬断；
  其他既有启动参数会原样保留并去重。
- 桌面端在应用日志目录写入轮转 `diagnostics.jsonl`（512 KiB × 当前+4 份），仅记录时间、
  request ID、阶段、host/path、错误码、HTTP 状态和重试次数，不记录 Cookie、token 或 body。
