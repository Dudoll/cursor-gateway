...
手工：CS「启用加密」→ Secure 配对（`PAIRING_MAIL_MODE=log` + `scripts/e2ee/read-pairing-mail.sh`）→ 自动回 CS fragment → 右上角出现「本次聊天已加密」→ 加密发送。反复测配对时：点右上角低调的「退出加密」（确认后清 IndexedDB / 待授权状态，可选退出 CF Access），徽章回到未授权后再点「启用加密」即可从头走一遍。
