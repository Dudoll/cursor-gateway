...
3. Secure 完成（或复用）magic-link 配对后，向 Gateway 标记 `pending_runner`；Runner 认领、登记 CS 公钥、签发授权；Secure 轮询到 grant 后 `location.replace` 回 CS：`#cs_auth=<base64url JSON>`。  

**手机注意**：邮件里的 magic link 请尽量用**同一浏览器**打开（与 CS / Secure 同一套存储）。Gmail App 内置浏览器可能另开标签或隔离存储；CS→Secure 的回跳上下文（`return_origin` / challenge / state / 设备指纹）已持久化到 Secure origin 的 `localStorage`（带 TTL、单次使用），同浏览器其它标签完成配对后仍会自动回 CS。若提示上下文过期或缺失，请回 CS 重新点「启用加密」。
...
`E2EE_REQUIRED_FOR_WEB=true` 时：**登录 CS 后自动进入加密授权**（跳转 Secure），未完成设备授权前禁用发送；明文 `POST /api/runs` / Memory 返回 **426**。新消息一律 E2EE；侧栏与时间线**合并**历史明文会话与加密会话（按时间），历史明文低调标记「历史明文」，不诱导再开明文模式。

未授权时顶栏显示「完成加密授权」；授权成功后显示「本次聊天已加密」。
...
| `E2EE_REQUIRED_FOR_WEB` | **`true`（生产）**：登录后自动跳转授权；未配对禁用明文。回退：改 `false` 并重建 app。 |
...
手工验收：登录 CS → 自动跳 Secure 配对（`PAIRING_MAIL_MODE=log` 或真实邮件）→ 手机尽量同浏览器打开 magic link → 自动 `#cs_auth=` 回 CS → 右上角「本次聊天已加密」→ 加密发送。侧栏应同时看到历史明文（标「历史明文」）与加密会话。反复测：徽章面板「退出加密并重新配对」后应再次自动引导授权。

### 回退强制加密

VPS `.env` 设 `E2EE_REQUIRED_FOR_WEB=false`，保留其它密钥，`docker compose` 重建 app 即可恢复可选加密 / 明文路径。
...
- 默认打开 `E2EE_REQUIRED_FOR_WEB` 却不提供可靠自动授权回跳（会导致未配对用户锁死）。