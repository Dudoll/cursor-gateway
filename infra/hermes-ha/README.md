...
轻量 `hermes-ha-host-load.timer`（默认每 2 分钟）采集 CPU/内存/swap/load、
Docker app/PG cgroup、Hermes unit 资源与脱敏 top 进程，落盘到
`~/.local/state/hermes-ha/host-load/`（`latest.json` + 滚动 `samples/`，默认保留
288 份）。失败 fail-open，不切主备。手动跑：

```bash
python3 ~/hermes-ha/scripts/sample-host-load.py
# 或仓库内
