...
3. 写本机配置 `~/.cursor-gateway/secure-adapter.env`（0600，含**真实 key**）+ 启动器 `start-secure-adapter.sh`。
4. **幂等**写 shell 受管块：把 CLI 的 `ANTHROPIC_*/OPENAI_*` 指向本机 Adapter（用本地 loopback key，非真实 key）。

```bash
# 交互式（提示输入真实 key）
sh install-csapi-secure.sh
# 非交互 + 立即后台启动 Adapter
CSAPI_API_KEY=sk-xxxx sh install-csapi-secure.sh --start
# 只打印 / 卸载 / 状态 / 停止 / 跳过探测
...
Windows：`powershell -ExecutionPolicy Bypass -File .\install-csapi-secure.ps1`（`-Start / -Print /
-Uninstall / -Status / -NoProbe`）。
...
| 依赖 | 零依赖，**单文件可分发** | 需仓库源码 + `npm install`（node≥22）跑 Adapter |
...
- 安全版**可以** `curl|sh` 完成「配置 + 探测 + 核对根指纹」，但**启动 Adapter 仍需仓库源码**
  （`apps/secure-adapter` + `npm install`）。所以典型用法是：先 clone 仓库，再在仓库内跑本脚本；
  或 `curl|sh` 预置配置后设 `CSAPI_REPO_DIR=/path/to/repo` 指向已 clone 的仓库启动。
