...
export function buildRunnerEnvSnippet(opts) {
  ...
  // Pairing mail: keep log until operator adds MAIL_API_KEY to the runner .env.
  values.PAIRING_MAIL_MODE = "log";
  values.PAIRING_MAIL_FROM = "no-reply@piallera.com";
  values.PAIRING_MAIL_FROM_NAME = "Piallera Secure";
  values.MAIL_API_PROVIDER = "resend";
  return serializeEnv(
    values,
    "Generated by foolproof-deploy — fill CURSOR_API_KEY and RUNNER_WORKSPACES. For production pairing mail set PAIRING_MAIL_MODE=api and MAIL_API_KEY in the gitignored runner .env (see docs/pairing-mail.md). PAIRING_MAIL_TO is test-script only."
  );
}
